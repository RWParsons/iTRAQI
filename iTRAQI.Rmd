...
lzn_fit_acute <- fit.variogram(lzn_vgm, model=vgm("Gau"))
# plot(lzn_vgm, lzn_fit)

# rehab variogram
lzn_vgm <- variogram(rehab_time~1, df_times)
lzn_fit_rehab <- fit.variogram(lzn_vgm, model=vgm("Gau"))
# plot(lzn_vgm, lzn_fit)
...
krige(acute_time ~ 1, df_times, pnts, model=lzn_fit_rehab)%>%
  as.data.frame() %>%
  ggplot(aes(X, Y)) + 
  geom_tile(aes(fill=var1.pred)) + 
  coord_equal()+
...
bins <- c(0, 30, 60, 120, 180, 240, 300, 360, Inf)
pal <- colorBin("YlOrRd", domain = SA1s_acute$mean, bins = bins)
