...
qld_SAs <- st_read("input/qld_sa_zones_2016/MB_2016_QLD.shp")

as.data.frame(qld_SAs) %>%
  dplyr::select(SA2_MAIN16, SA2_NAME16) %>%
  distinct() %>%
  write.csv("input/SA2s_names_lookup.csv", row.names = FALSE)

as.data.frame(qld_SAs) %>%
  dplyr::select(SA1_MAIN16, SA1_NAME16=SA2_NAME16) %>%
  distinct() %>%
  write.csv("input/SA1s_names_lookup.csv", row.names = FALSE)
# ggplot(data = world) +
#   geom_sf() +
#   geom_sf(data=qld_SAs, aes(fill=SA3_NAME16), col = NA) +
#   coord_sf(xlim = c(100.00, 160.00), ylim = c(-45.00, -10.00), expand = T) + 
#   guides(fill="none")
# aust_SAs <- st_read("C:/Users/Rex/Downloads/MB_2021_AUST_SHP_GDA2020/MB_2021_AUST_GDA2020.shp")
```
...
SA1s_acute <- readRDS("input/layers/SA2s_acute.rds") %>%
  mutate(popup = paste0(
    paste0(
      "<b>ID: </b>", .[[1]], "<br>",
      "<b>Time to care (minutes): </b>", .[[2]], "<br>"
    )
  ))

lookup <- read.csv("input/SA2s_names_lookup.csv")
lookup$SA2_MAIN16 <- as.character(lookup$SA2_MAIN16)
SA1s_acute %>%
  left_join(., lookup)
